"""기능 하나의 판정. 복사해서 쓰는 원본이다.
    ...
    cp src/<pkg>/features/template.py src/<pkg>/features/<기능>.py

기능이 하나뿐인 동안은 이 폴더를 쓰지 않는다 — `pipeline.py` 에 그대로 짠다.
둘째 기능이 생기는 순간 첫 기능도 여기로 옮기고, `pipeline.py` 는 기능들을 불러
합치는 자리가 된다 (§1.5).

**이 파일 자체는 아무도 부르지 않는다.** `pipeline.py` 가 import 하는 것은 복사해서
만든 파일들이다. 원본을 지우지 마라 — 일곱 번째 기능을 만드는 사람이 기존 기능
하나를 골라 베끼면 그 기능만의 사정까지 따라간다.

지켜야 하는 것 둘:

- **`process_data(rows) -> dict` 로 노출한다.** `pipeline.py` 가 이 이름으로 부른다
- **지표 이름 앞에 `NAME` 을 붙인다.** 기능 여럿의 결과가 한 리포트에 모이므로(§3.2)
...
NAME = "template"  # 파일 이름과 같게 둔다. 지표 접두어로 쓰인다
