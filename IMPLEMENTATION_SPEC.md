...
한 저장소에서 여러 기능(A, B, C...)을 동시에 개발할 때:

### 초기 설정
```bash
# adopt.py로 시작 (몇 개 프로젝트 필요한지 미리 알면)
python adopt.py
# Project 1: A
# Project 2: B
# → src/A/, src/B/ 생성
...
```bash
# src/template/ 을 복사
cp -r src/template src/C
cp -r src/template src/D

# 각각에서 main.py, pipeline.py 커스터마이징
```

**구조:**
```
src/
  framework/     (모든 프로젝트가 공유)
  template/      (복사 용도 템플릿)
  A/             (프로젝트 A)
  B/             (프로젝트 B)
  C/, D/, ...    (필요할 때 추가)
...
**실행:**
각 프로젝트마다 독립적인 진입점 필요 (run.py를 수정하거나, 각 프로젝트별 run.py 생성).
