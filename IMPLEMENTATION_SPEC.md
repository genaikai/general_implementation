...
src/<pkg>/
  ...
  features/
    template.py     ← 복사 원본. 저장소에 둔다
    <기능>.py       ← 기능마다 파일 하나. 판정 로직만
```

**기능 하나에 파일 하나다.** 폴더를 주지 않는 이유는 아래 규칙 때문이다 — 기능이 가질 수
있는 것이 판정 로직뿐이라, 폴더를 주면 파일 하나만 든 껍데기가 된다. 기능이 자기만의
헬퍼를 갖게 되면 그때 폴더로 올린다.

**기능 파일에는 판정 로직만 둔다.** 인자 파싱·적재·리포트를 기능마다 복사하면 기능별로 따로
...
> `adopt.sh`는 `src/mypkg/` 아래를 통째로 훑으므로 하위 폴더가 늘어도 그대로 동작한다.
...
`src/mypkg/` 를 그 프로젝트에 맞는 이름으로 갈아엎는 바람에 `adopt.sh` 가 찾을 디렉터리가
