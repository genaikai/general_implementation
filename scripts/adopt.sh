...
- **기능 코드는 \`src/$PKG/pipeline.py\`.** 포맷을 읽는 코드는 \`load.py\`,
  그 외에는 손댈 일이 거의 없다
- **기능이 둘 이상이 되면** \`src/$PKG/features/<기능>.py\` 로 가른다
  (\`features/template.py\` 를 복사한다). \`pipeline.py\` 는 그것들을 불러 합치는
  자리가 되고, 공유 코드와 진입점은 그대로 하나다 — 기능마다 진입점을 두면 리포트가
  여러 장으로 갈라지고, 결과 파일을 못 가져오는 환경에서는 그걸 합칠 방법이 없다.
